...
function CheckAuthentication()
{
	...
	return true;//conment lai khi kiem tra login
	
	/*Đây là  hàm ckfinder dùng để kiểm tra xem người dùng có đăng nhập vào hệ thống hay chưa, 
	nếu có rồi thì mới cho người dùng upload, còn không thì sẽ hiển thị ra thông báo lỗi như ở trên.
	Nếu trong hàm này,bạn chỉ có một dòng code là return true, thì đồng nghĩa với việc không cần kiểm tra  login.
	Giả sử trong website của bạn, để kiểm tra xem người dùng có đăng nhập hay chưa, bạn kiểm tra dựa vào $_SESSION['LOGIN'] == ‘DONE’ 
	thì ta có thể chỉnh lại hàm trên như sau
	
    if (!isset($_SESSION['idNguoiDungQT'])) return true;
    else return false;
